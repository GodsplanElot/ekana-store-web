  ...
  return <section className="px-4 py-8 sm:px-7 lg:px-10 lg:py-12"><div className="mx-auto max-w-6xl">
    ...
    <div className="mt-8 grid gap-6 lg:grid-cols-[minmax(0,1fr)_320px]">
      ...
      <aside className="space-y-5">
        <OrderStatusForm initialFulfillmentStatus={order.fulfillment_status} initialPaymentStatus={order.payment_status} reference={order.reference} />
        <section className="border border-stone-900/15 bg-[#fffdf9] p-5">
          <h2 className="font-serif text-xl">Payment record</h2>
          <dl className="mt-4 space-y-4 text-xs">
            <div>
              <dt className="font-semibold uppercase tracking-[0.14em] text-stone-500">Paystack reference</dt>
              <dd className="mt-1.5 break-all leading-5 text-stone-700">{order.paystack_reference ?? "Not available"}</dd>
            </div>
            <div>
              <dt className="font-semibold uppercase tracking-[0.14em] text-stone-500">Confirmation source</dt>
              <dd className="mt-1.5 capitalize leading-5 text-stone-700">{order.payment_confirmation_source?.replaceAll("_", " ") ?? "Not recorded"}</dd>
            </div>
            <div>
              <dt className="font-semibold uppercase tracking-[0.14em] text-stone-500">Failure reason</dt>
              <dd className="mt-1.5 break-words leading-5 text-stone-700">{order.payment_failure_reason?.replaceAll("_", " ") ?? "None recorded"}</dd>
            </div>
          </dl>
        </section>
      </aside>
